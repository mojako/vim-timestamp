...
if !exists('g:timestamp_prefix')
  let g:timestamp_prefix = '\%(Changed\?\|Modified\|Updated\?\)\%([ -]*Date\)\?'
endif
...
function! timestamp#overwrite()
  let pat = g:timestamp_prefix . '[ :]*\zs.\{-}' . g:timestamp_suffix
  ...
  while i <= 20
    ...
    if m >= 0 && synIDattr(synID(i, m, 0), 'name') =~? 'Comment'
      ...
      let newline = substitute(line, pat, s:dt.now().format(str, 'en_US'), '')
      if line != newline
